...
extension SignalProtocol {
    ...
    public static func heartbeat(interval seconds: Double) -> Signal<Void, Never> {
        ...
        return willEnterForeground.replace(with: ()).start(with: ()).flatMapLatest { () -> Signal<Void, Never> in
            return Signal<Int, Never>.interval(seconds, queue: .global()).replace(with: ()).start(with: ()).take(until: didEnterBackgorund)
        }
    }
}
#endif
